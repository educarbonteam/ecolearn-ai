...
async def generate_course(
    ...
    db_course = Course(
        ...
        category=request.topic.split()[0],
        difficulty=request.difficulty,
        duration=request.duration,
...
def update_progress(
    ...
    if enrollment.progress >= 100 and not enrollment.completed:
        ...
        carbon_calc = CarbonService.calculate_carbon_footprint(
            float(course.duration.replace("h", "").strip()),
            course.category
        )
        current_user.carbon_offset += carbon_calc["carbon_offset_kg"]
        current_user.trees_planted += carbon_calc["trees_equivalent"]
...
async def plant_trees(
    ...
    plantation = TreePlantation(
        ...
        api_transaction_id=result["transaction_id"],
        status=result["status"],
        carbon_equivalent=result["carbon_offset_kg"]
    )
    db.add(plantation)
    current_user.trees_planted += request.trees_count
    current_user.carbon_offset += result["carbon_offset_kg"]
    db.commit()
